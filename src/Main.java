...
public class Main {
	...
	public static void main(String[] args) {
		try {
			if (!Arquivo.existe(arqAdmin)) {
				System.out.println("Percebemos que é a primeira vez que executas o programa..");
				System.out.print("Informe (Sem espaços) o nome do Administrador: ");
			...
			System.err.println("Ocorreu um erro inesperado, estamos finalizando a aplicação.");
			ex.printStackTrace();
		}
	}
	...
	private static void areaAdministrador() throws IOException {
		...
		do {
			...
			System.out.println("3. Editar Avião");
			System.out.println("4. Remover Avião");
			System.out.println("5. Consultar Aviões cadastrados");
			...
			case 1:
				...
				break;
			case 2:
				System.out.println("informe a quantidade de avião que deseja cadastrar:");
				try {
					int qtd = scan.nextInt();
					for (int i = 0; i < qtd; i++) {
						scan.nextLine();
						System.out.print("Informe o idAviao do avião: ");
						String idAviao = scan.nextLine();
						idAviao = idAviao.trim();
						System.out.print("Informe o destino do avião: ");
						String destino = scan.nextLine();
						destino = destino.trim();
						System.out.print("Informe (hh:mm) o horário de partida: ");
						String horario = scan.next();
						if (!Validacao.horario(horario)) {
							System.out.println("Horário Inválido");
							while (!Validacao.horario(horario)) {
								System.out.println("Informe um horário válido! (hh:mm)");
								horario = scan.next();
							}
						}
						...
						System.out.println("Avião registrado");
					}
				} catch (Exception e) {
			...
			case 3:
				if (aeroporto.getAvioes().isEmpty()) {
					System.err.println("Não há aviões para editar");
				} else {
					...
					do {
						...
						if (cont == 0) {
							System.err.print("Avião não encontrado.\n\n");
							cont = aeroporto.getAvioes().size();
						}
					} while (!sairId);
					try {
						System.out.print("Informe o novo Id do avião: ");
						idEditado = br.readLine();
						System.out.print("Informe o destino do avião: ");
						...
						System.out.println("Avião editado");
						sairId = true;
					} catch (Exception e) {
			...
			case 4:
				if (aeroporto.getAvioes().isEmpty()) {
					System.err.println("Não há aviões para remover!");
				} else {
					scan.nextLine();
	...
	private static void areaCliente() throws IOException {
		...
		while (op != 69) {
			...
			case 1:
				if (aeroporto.getAvioes().isEmpty()) {
					System.err.println("Nao há aviões, chame o administrador");
				} else {
					Reservar();
				}
				break;
	...
	public static void Reservar() {
		...
		try {
			...
			do {
				...
				if (auxcpf.length() != 11) {
					while (true) {
						System.out.println("CPF inválido. Informe um válido!");
				...
				do {
					...
					for (int k = 0; k < aeroporto.getAvioes().size(); k++) {
						if (aeroporto.getAvioes().get(k).getIdAviao().equals(idAviao)) {
							...
							if (aeroporto.getAvioes().get(indice).getDisponiveis() == 0) {
								System.err.println("Não há cadeiras disponíveis");
								sairId = false;
							}
							break;
						} else {
					...
					if (cont == 0) {
						System.err.println("Avião não encontrado.");
					}
					...
					try {
						if (Integer.parseInt(l) > aeroporto.getAvioes().get(indice).getAltura()
								...
								|| aeroporto.getAvioes().get(indice).getPoltronas()[Integer.parseInt(l)][Integer
										.parseInt(c)] == true) {
							sairPoltrona = false;
							System.err.println("Poltrona Inválida!");
						} else {
	...
	public static void Consultar() {
		...
		try {
			...
			case 1:
				...
				if (cont == 0) {
					System.err.println("Não tem avião com esse destino!");
				}
				break;
			case 2:
				scan.nextLine();
